...
class SavedComparisonSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'user', 'products', 'product_ids', 'name', 'created_at']

    def create(self, validated_data):
        user = self.context['request'].user
        return SavedComparison.objects.create(user=user, **validated_data)
...
class PriceAlertSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = PriceAlert
        fields = ['id', 'user', 'product', 'product_id', 'target_price', 'is_active', 'created_at']

    def create(self, validated_data):
        user = self.context['request'].user
        product = validated_data['product']
        if PriceAlert.objects.filter(user=user, product=product, is_active=True).exists():
            raise serializers.ValidationError("Vous avez déjà une alerte active sur ce produit.")
